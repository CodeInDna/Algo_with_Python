...
# it wraps over to the other side. For instance, a jump of -1 at index 0 brings us to the last 
# index in the array. Similarly, a jump of 1 at the last index in the array brings us to index 0. 
...
def hasSingleCycle(lst):
	currentIdx = 0
	numElementsVisited = 0
	while numElementsVisited < len(lst):
		if numElementsVisited > 0 and currentIdx == 0:
			return False
		numElementsVisited += 1
		currentIdx = getNextIdx(currentIdx, lst)
	return currentIdx == 0

def getNextIdx(currentIdx, lst):
	jump = lst[currentIdx]
	nextIdx = (jump + currentIdx) % len(lst)
	return nextIdx if nextIdx >= 0 else nextIdx + len(lst)
# ----------------METHOD 01---------------------#
